                ...
                script {
                    CHANGED_SERVICES.split(",").each { service ->
                        dir(service) {
                            ...
                            sh "../mvnw clean install -Dmaven.test.skip=true -P buildDocker -Ddocker.image.prefix=${env.DOCKER_REGISTRY} -Dcontainer.image.tag=${LATEST_COMMIT} -Dcontainer.build.extraarg=\\\"--push\\\""
                        }
                    }
                }
            }
